...
const DEFAULT_BUSINESS = {
  ...
  phone: '699 24 23 76',
  address: 'WG2M+HH2, Yaoundé, Cameroun',
  landmark: 'Près de Neptune Oil · Chapelle Manguier',
...
// Vue client : aucune donnée personnelle superflue n'est exposée.
const publicOrder = o => ({
  code: o.code,
...
// Aucun mot de passe de repli n'est codé en dur : sans ADMIN_PASSWORD
// et sans mot de passe enregistré, la connexion est impossible.
...
// Le drapeau Secure suit le protocole afin que le développement local en HTTP fonctionne.
...
async function ensureInitialized() {
  ...
  // Reprise des commandes existantes : création de l'index code -> identifiant.
...
// Les images importées sont stockées séparément : le catalogue ne transporte
// plus de base64 et reste léger même avec de nombreux produits.
async function persistImage(value, previous) {
  ...
  if (buffer.length > MAX_BLOB_BYTES) {
    throw new Error('Image trop lourde. Limite Netlify Blobs : 5 Mo par donnée.');
  }
...
// Recherche indexée : une seule lecture au lieu d'un parcours complet.
  ...
  try {
    ...
    if (path === '/api/auth/login' && req.method === 'POST') {
      ...
      if (!saved && !ADMIN_PASSWORD) {
        return json({
          error: "Configuration incomplète : la variable d'environnement ADMIN_PASSWORD doit être définie sur Netlify."
        }, 503);
      }

      const throttle = await loginThrottle(req);
      if (throttle.blocked) {
        return json({ error: `Trop de tentatives. Réessayez dans ${throttle.wait} minute(s).` }, 429);
      }
    ...
    if (path === '/api/admin/security/password' && req.method === 'PUT') {
      ...
      if (String(b.newPassword || '').length < MIN_PASSWORD_LENGTH) {
        return json({ error: `Le nouveau mot de passe doit contenir au moins ${MIN_PASSWORD_LENGTH} caractères.` }, 400);
      }
    ...
    // Vue gérant : inclut les produits masqués afin de pouvoir les réactiver.
    ...
    if (path === '/api/products' && req.method === 'POST') {
      ...
      if (!String(b.name || '').trim() || !String(b.category || '').trim()) {
        return json({ error: 'Nom et catégorie requis.' }, 400);
      }
    ...
    if (pm && req.method === 'PUT') {
      ...
      const r = await setJSON(key, p, { onlyIfMatch: got.etag });
      if (!r.modified) {
        return json({ error: 'Le produit a été modifié simultanément. Rechargez puis réessayez.' }, 409);
      }
    ...
    if (path === '/api/orders' && req.method === 'POST') {
      ...
      try {
        ...
        return json({ error: e.message || 'Impossible d’enregistrer la commande.' }, 400);
      }
    ...
    if (rm && req.method === 'POST') {
      ...
      // Seule une commande livrée peut être confirmée par le client.
      if (o.status !== 'delivered') {
        return json({ error: 'Cette commande ne peut pas encore être confirmée.' }, 409);
      }
      o.received = true;
      o.status = 'received';
    ...
    console.error(e);
    if (e?.message === 'JSON invalide') return json({ error: 'Requête invalide.' }, 400);
